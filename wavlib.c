...
int GetListChunk(int hFilename,struct wavList *wavList)
{
  struct wavChunk wavChunk;
  int i;
  unsigned char cBuffer[LIST_SIZE];
  off_t oLimit,oOffset;
  char sName[5]={0,0,0,0,0};
  unsigned dBuffer[4];
  unsigned char sBuffer[LIST_SIZE];
  off_t oTemp,oSize;
  int dReturn=1;
...
int WriteSword(int hFilename,unsigned dValue)
{
...
int WriteDword(int hFilename,unsigned dValue)
{
...
int GetMpegFormat(unsigned char *header,struct wavMpegFmt *wavMpegFmt)
{
  int mpeg_id=1;
  ...
  switch(mpeg_id) {
      case 1:
	switch(wavMpegFmt->layer) {
	    case 1:
	      switch(header[2]>>4) {
		  case 1:
		    wavMpegFmt->bit_rate=32000;
		    break;
	
		  case 2:
		    wavMpegFmt->bit_rate=64000;
		    break;
	
		  case 3:
		    wavMpegFmt->bit_rate=96000;
		    break;
	
		  case 4:
		    wavMpegFmt->bit_rate=128000;
		    break;
	
		  case 5:
		    wavMpegFmt->bit_rate=160000;
		    break;
	
		  case 6:
		    wavMpegFmt->bit_rate=192000;
		    break;
	
		  case 7:
		    wavMpegFmt->bit_rate=224000;
		    break;
	
		  case 8:
		    wavMpegFmt->bit_rate=256000;
		    break;
	
		  case 9:
		    wavMpegFmt->bit_rate=288000;
		    break;
	
		  case 10:
		    wavMpegFmt->bit_rate=320000;
		    break;
	
		  case 11:
		    wavMpegFmt->bit_rate=352000;
		    break;
	
		  case 12:
		    wavMpegFmt->bit_rate=384000;
		    break;
	
		  case 13:
		    wavMpegFmt->bit_rate=416000;
		    break;
	
		  case 14:
		    wavMpegFmt->bit_rate=448000;
		    break;
	    ...
	    case 2:
		  ...
		  case 2:
		    wavMpegFmt->bit_rate=48000;
		    break;
	
		  case 3:
		    wavMpegFmt->bit_rate=56000;
		    break;
	...
	switch(wavMpegFmt->layer) {
	    ...
	    case 3:
	      switch(header[2]>>4) {
		  case 1:
		    wavMpegFmt->bit_rate=8000;
		    break;
	
		  case 2:
		    wavMpegFmt->bit_rate=16000;
		    break;
	
		  case 3:
		    wavMpegFmt->bit_rate=24000;
		    break;
   ...
   * Sample Rate
   */
  switch((header[2]>>2)&0x03) {
      case 0:
	switch(mpeg_id) {
	    case 1:
	      wavMpegFmt->sample_rate=44100;
	      break;

	    case 2:
	      wavMpegFmt->sample_rate=22050;
	      break;
	...
	switch(mpeg_id) {
	    case 1:
	      wavMpegFmt->sample_rate=48000;
	      break;

	    case 2:
	      wavMpegFmt->sample_rate=24000;
	      break;
	...
	switch(mpeg_id) {
	    case 1:
	      wavMpegFmt->sample_rate=32000;
	      break;

	    case 2:
	      wavMpegFmt->sample_rate=16000;
	      break;
  ...
  switch(header[3]>>6) {
      case 0:
	wavMpegFmt->mode=ACM_MPEG_STEREO;
	wavMpegFmt->channels=2;
	break;

      case 1:
	wavMpegFmt->mode=ACM_MPEG_JOINTSTEREO;
	wavMpegFmt->channels=2;
	break;

      case 2:
	wavMpegFmt->mode=ACM_MPEG_DUALCHANNEL;
	wavMpegFmt->channels=2;
	break;

      case 3:
	wavMpegFmt->mode=ACM_MPEG_SINGLECHANNEL;
	wavMpegFmt->channels=1;
	break;
  }
  ...
  wavMpegFmt->flags=0;
  if((header[2]&0x01)!=0) {
    wavMpegFmt->flags|=ACM_MPEG_PRIVATEBIT;
  }
  if((header[3]&0x08)!=0) {
    wavMpegFmt->flags|=ACM_MPEG_COPYRIGHT;
  }
  if((header[3]&0x04)!=0) {
    wavMpegFmt->flags|=ACM_MPEG_ORIGINALHOME;
  }
  if((header[1]&0x01)!=0) {
    wavMpegFmt->flags|=ACM_MPEG_PROTECTIONBIT;
  }
  if(mpeg_id==1) {
    wavMpegFmt->flags|=ACM_MPEG_ID_MPEG1;
  }

  /*
   * Frame Size (without padding)
   */
  wavMpegFmt->frame_size=144*wavMpegFmt->bit_rate/wavMpegFmt->sample_rate;
